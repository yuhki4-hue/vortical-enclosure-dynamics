"""Critical-D analysis with conserved rotational energy."""
...
CSV_PATH = SCRIPT_DIR / "dynamic_barrier_critical_D_no_rotation_damping.csv"
...
def dynamic_barrier_system(_t, y, d_value):
    ...
    dq_dt = v
    # This experiment isolates the role of velocity dissipation in stabilizing
    # quasi-closure when rotational energy is not damped.
...
def save_instability_plot(rows, critical_d):
    ...
    fig.tight_layout()
    fig.savefig(
        OUTPUT_DIR / "fig_barrier_09_critical_threshold.png",
        dpi=300,
    )
    plt.close(fig)
...
def save_amplitude_plot(rows):
    ...
    fig.tight_layout()
    fig.savefig(
        OUTPUT_DIR / "fig_barrier_07_oscillation_amplitude.png",
        dpi=300,
    )
    plt.close(fig)
...
def save_closure_plot(rows):
    ...
    fig.tight_layout()
    fig.savefig(
        OUTPUT_DIR / "fig_barrier_08_closure_statistics.png",
        dpi=300,
    )
    plt.close(fig)
