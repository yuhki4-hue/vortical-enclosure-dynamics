...
def run(seed, lam, gamma, variant, epochs):
    ...
    nsub = ndead = ndyad = 0
    act = []; audit_ok = True
    branch_samples = []; prev_ledger_events = 0
    snap_results = []
    ...
    for ep in range(epochs):
        ...
        for L in links:
            ...
            events += [L]*int(k)
        n_ledger_events = len(events)          # children of last epoch
        if ep > 20 and prev_ledger_events > 0:
            branch_samples.append(n_ledger_events / prev_ledger_events)
        prev_ledger_events = max(n_ledger_events, 0)
        ...
        if len(events) > 100_000:
            return "RUNAWAY", ep, nsub, ndead, ndyad, act, audit_ok, np.nan, np.nan, snap_results
        ...
        if (ep + 1) in SNAPS:
            ...
            if len(idx2) >= 3:
                snap_results.append((ep + 1,) + panel(C[np.ix_(idx2, idx2)], len(idx2)))
            else:
                snap_results.append((ep + 1, "DEAD", np.nan, np.nan, np.nan, np.nan, np.nan, 0))
    idx = np.where(alive)[0]
    if len(idx) < 3:
        return "EMPTY", epochs, nsub, ndead, ndyad, act, audit_ok, np.nan, np.nan, snap_results
    deg = (C[np.ix_(idx, idx)] > LINK_EPS).sum(1)
    hub = float(deg.max()/max(deg.mean(), 1e-9))
    br = float(np.mean(branch_samples)) if branch_samples else np.nan
    res = panel(C[np.ix_(idx, idx)], len(idx))
    return res, epochs, nsub, ndead, ndyad, act, audit_ok, hub, br, snap_results

mode = sys.argv[1]
if mode == "track":
    lam, g = float(sys.argv[2]), float(sys.argv[3])
    seeds = [int(s) for s in sys.argv[4].split(",")]
    for seed in seeds:
        out = run(seed, lam, g, "C", epochs=300)
        snaps = out[-1]
        cells = []
        for s in snaps:
            ep = s[0]
            if s[1] == "DEAD":
                cells.append(f"ep{ep}:DEAD")
            else:
                ph, medC, f09, gcf, d, cv, cyc = s[1:]
                ds = f"{d:.2f}" if isinstance(d, float) and np.isfinite(d) else "-"
                cells.append(f"ep{ep}:{ph.strip()}(C={medC:.2f},d={ds})")
        status = out[0] if isinstance(out[0], str) else "ok"
        print(f"  (l={lam},g={g}) s={seed} [{status}]: " + "  ".join(cells))
