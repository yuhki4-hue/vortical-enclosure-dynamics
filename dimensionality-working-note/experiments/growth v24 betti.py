...
def run(seed, lam, gamma, directed, epochs=300, protect=False):
    ...
    for ep in range(epochs):
        ...
        if len(events) > 100_000:
            return "RUNAWAY", snaps, asym_hist, nsub, ndead, audit_ok, evledger, decledger, deathledger
        Wnext = {}
        rng.shuffle(events)
        for (i, j) in events:
    ...
    return "ok", snaps, asym_hist, nsub, ndead, audit_ok, evledger, decledger, deathledger
...
for seed in seeds:
    status, snaps, asym, nsub, ndead, audit, evl, decl, deathl = run(seed, lam, g, True, epochs=epochs, protect=False)
