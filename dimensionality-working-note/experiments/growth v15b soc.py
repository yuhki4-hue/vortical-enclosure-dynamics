"""v15b (endogenous Poisson rates per 5.27.4): gradient-driven link-event model (pre-registered 5.26).
...
def run(seed, lam, gamma, epochs, ev_per_epoch):
    ...
    for ep in range(epochs):
        ...
        if tot < 1e-12:
            ...
            counts[rng.integers(len(li))] = 1   # minimal symmetry-breaking quantum
        else:
            counts = rng.poisson(wts)           # mean |d rho| * dt, dt = 1
...
if mode == "scan":
    ...
    for lam, g in ((0.9, 0.25), (0.7, 0.2)):
            out = run(5, lam, g, epochs=100, ev_per_epoch=0)
            if out[0] is None:
