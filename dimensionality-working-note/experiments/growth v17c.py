...
def run(seed, lam, gamma, variant, epochs):
    ...
    for ep in range(epochs):
        ...
        if len(events) > 100_000:
            return "RUNAWAY", ep, nsub, ndead, ndyad, act, audit_ok, np.nan, np.nan
    ...
    idx = np.where(alive)[0]
    if len(idx) < 3:
        return "EMPTY", epochs, nsub, ndead, ndyad, act, audit_ok, np.nan, np.nan
    ...
    return res, epochs, nsub, ndead, ndyad, act, audit_ok, hub, br

mode = sys.argv[1]
if mode == "compare":
    variant = sys.argv[2]
    lam, g = float(sys.argv[3]), float(sys.argv[4])
    for seed in (5, 6, 7):
        out = run(seed, lam, g, variant, epochs=200)
        res, ep, nsub, ndead, ndyad, act, audit, hub, br = out
        a3 = np.mean(act[-len(act)//3:]) if act else 0
        if res in ("RUNAWAY", "EMPTY"):
            print(f"  {variant} (l={lam},g={g}) s={seed}: {res}")
            continue
        ph, medC, f09, gcf, d, cv, cyc = res
        ds = f"{d:5.2f}" if np.isfinite(d) else "  nan"
        cs = f"{cv:4.2f}" if np.isfinite(cv) else " nan"
        print(f"  {variant} (l={lam},g={g}) s={seed}: {ph} medC={medC:4.2f} f09={f09:4.2f} "
              f"d={ds} cv={cs} cyc={cyc:4d} sub={nsub:3d} dead={ndead:3d} "
              f"hub={hub:5.1f} sigma={br:5.3f} act3={a3:6.1f} audit={'OK' if audit else 'FAIL'}")
