"""v24.2: headroom run (Working Note II sec 12-1, post v24.1 reading).
...
def report(tag, out):
    ...
    print(f"    ep    ph    V  b1_H12 b1_H6 b1_S12 b1_S6  occSCC occSCC3 feCYC feDYAD maxSCC rung12 refl ndstM  gcf  npts wscale wcont slope   r2    fail")
    for r in inst:
        d = r['diag']
        print(f"    {r['ep']:4d} {r['ph']:>5s} {r['V']:4d}  "
              f"{r['b1_H12']:5d} {r['b1_H6']:5d} {r['b1_S12']:5d} {r['b1_S6']:5d}  "
              f"{_f(r['occ_scc'])} {_f(r['occ_scc3'])}  "
              f"{_f(r['fe_cyc'])} {_f(r['fe_dyad'])} "
              f"{r['max_scc']:5d} {_f(r['rung12'])} {r['refl']:5d} {_f(r['ndst_mean'])} "
