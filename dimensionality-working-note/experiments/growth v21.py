...
globals()['SNAPS'] = tuple(range(25, epochs + 1, 25))
for seed in seeds:
    ...
    for s in snaps:
        ...
        best = max(best, cur)
    if cur: runs.append(cur)
    # branch tagging: cooling if medC decreased vs previous snapshot
    cool = heat = 0
    for a_, b_ in zip(snaps, snaps[1:]):
        if b_[1] == "METR" and isinstance(a_[2], float) and isinstance(b_[2], float):
            if b_[2] < a_[2]: cool += 1
            else: heat += 1
    trace = " ".join(f"{s[0]}:{s[1]}" for s in snaps)
    print(f"g={g} s={seed} [{status}] occ={occ}/{nsn} maxrun={best} runs={runs} "
          f"branch(cool/heat)={cool}/{heat} sub={nsub} dead={ndead} audit={'OK' if audit else 'FAIL'}")
