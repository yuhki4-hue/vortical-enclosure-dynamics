    ...
    for ep in range(epochs):
        ...
        if (ep + 1) in SNAPS:
            ...
            if len(idx2) >= 3:
                ...
                snaps.append((ep+1, "DEAD", np.nan, np.nan, np.nan, 0))
    return "ok", snaps, asym_hist, nsub, ndead, audit_ok

lam, g = 0.9, 0.15
mode = sys.argv[1]           # "prot" or "ctrl"
seeds = [int(s) for s in sys.argv[2].split(",")]
for seed in seeds:
    status, snaps, asym, nsub, ndead, audit = run(seed, lam, g, True, protect=(mode=="prot"))
    occ = sum(1 for s in snaps if s[1] == "METR")
    best = cur = 0
    for s in snaps:
        cur = cur + 1 if s[1] == "METR" else 0
        best = max(best, cur)
    pc_vals = [s[4] for s in snaps if len(s) > 4 and isinstance(s[4], float) and np.isfinite(s[4])]
    tri = [s[5] for s in snaps if len(s) > 5]
    pm = np.mean(pc_vals) if pc_vals else np.nan
    trace = " ".join(f"{s[0]}:{s[1]}" for s in snaps)
    print(f"{mode} s={seed} [{status}] occ={occ}/12 maxrun={best} Phi_circ={pm:5.3f} "
          f"ntri~{int(np.mean(tri)) if tri else 0} sub={nsub} dead={ndead} audit={'OK' if audit else 'FAIL'}")
    print(f"   {trace}")
