...
NMAX = 300; PB = 0.02; THETA = 0.75; K = 2
LAM = 0.3; GAMMA = 0.05
TOTAL_EVENTS = 150_000; EVENTS_PER_EPOCH = 500
...
print(f"v14b (time-budget per 5.24.3) de-registration, cell (l={LAM},g={GAMMA}); control v13 = SAT (5.22)")
import sys
_pc = float(sys.argv[1]); _seeds = [int(s) for s in sys.argv[2].split(",")]
for pc in (_pc,):
    for seed in _seeds:
        out = run(seed, pc)
        if out[0] is None:
