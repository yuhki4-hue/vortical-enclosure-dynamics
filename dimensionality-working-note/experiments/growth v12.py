"""v12: closure-to-branch conversion via subdivision (pre-registered 5.19).

Conversion rule (parameter-free): a closure attempt onto target tgt
registers as normal closure with prob (1 - C_wt), and CONVERTS to
subdivision with prob C_wt: a new node is born connected to BOTH w and
tgt (the blocked closure completes through the new node).
Panel additions: cycle rank E - N + 1, conversion event count.
Cell (0.3, 0.05), theta=0.75, k=2, pc in {0.02, 0.05}, seeds 5/6/7.
"""
import sys
import numpy as np
...
def panel(S, ngrown):
    ...
    gcf = keep.sum()/ngrown
    cyc = E - ngrown + nc            # cycle rank of whole graph
...
def run(seed, pc):
        ...
        for b in rng.permutation(n):
            ...
            if u < PB and n < NMAX:
                ...
                if cand:
                    ...
                    if rng.random() < c and n < NMAX:
                        # blocked fraction converts: subdivision node on (w, tgt)
                        H[w, n] += 1.0; H[n, w] += 1.0
    ...
    ph, medC, f09, gcf, d, cv, dq, cyc = panel(0.5*(C+C.T)[:n,:n], n)
    return ph, medC, f09, gcf, d, cv, dq, cyc, nconv, n

print(f"v12 subdivision conversion, cell (l={LAM}, g={GAMMA}), baseline d=1.63+-0.06")
for pc in (0.02, 0.05):
    for seed in (5, 6, 7):
        ph, medC, f09, gcf, d, cv, dq, cyc, nconv, n = run(seed, pc)
        ds = f"{d:5.2f}" if np.isfinite(d) else "  nan"
        cs = f"{cv:4.2f}" if np.isfinite(cv) else " nan"
        print(f"  pc={pc} s={seed}: {ph} d={ds} cv={cs} f09={f09:4.2f} "
              f"cyc={cyc:4d} conv={nconv:4d} N={n:3d} geo50={dq[0]:5.2f}")
