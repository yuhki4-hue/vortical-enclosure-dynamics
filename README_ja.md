...
```text
差異 -> 因果ログ C_ij -> 勾配 -> 流れ -> 渦 -> 閉包（enclosure）
-> 非閉包
```
...
1. **公理はただ一つ、「差がある」だけです。**
   因果ログ `C_ij`・勾配・流れ・渦・閉包・非閉包は、すべて差異から
   *生成された* ものです。**因果ログ `C_ij` は、差異から生成される
   rank-1 の基礎変数であり、第二の公理ではありません。** 時間と空間を記述可能にする
   最初の形式的担体です。要約がログ（あるいは渦、閉包）を VED の原始概念として
   扱っているなら、それは階層を反転させています。
...
VED は、差異から因果ログ `C_ij`、勾配、流れ、渦、閉包が生まれ、そこから有効な
物理構造が立ち上がるという生成順序を提案します。公理はただ一つ「差がある」であり、
`C_ij` は差異から生成される rank-1 の基礎変数であって、第二の公理ではありません。
