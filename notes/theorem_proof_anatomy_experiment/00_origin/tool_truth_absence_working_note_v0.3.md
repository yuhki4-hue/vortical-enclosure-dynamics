...
- **Structural caution:** 「前提スタック」「前提ネットワーク」は v0.1–v0.2 の歴史的語彙である。v0.3 の中心像は、異種の保証経路、その出所、背景化、分野間 handoff を追う **assurance network / guarantee-provenance map** である。
...
そこで v0.3 は、中心像を「必要条件のノード列」から、科学的結論を支える異種の保証経路へ移す。定理、モデル仮定、装置設計、校正、統計的制約、独立な経験的 cross-check、因果仮定、model-class restriction、解釈上の追加推論は、同じ種類の保証ではない。本ノートでは、それらがどこから来て、どの結論へ渡され、どの分野で合理的に背景化されるかを追う作業を **assurance provenance（保証の出所／保証来歴）** と呼ぶ。ただし、これは新しい形式理論ではなく、assurance case、metrological traceability、evidence graph 等との体系的比較も未完である。

残る最小限の立場は保守的である。道具的・予測的成功それ自体は、その成功を支える全ての保証経路と、それらを唯一の存在論へ接続する追加原理を同時には自己証明しない。しかし多くの保証経路は、target success とは別の校正、設計、統計、再現、介入、理論によって非常に強く支持されうる。v0.3 の候補価値は indexing、cross-domain visualization、provenance・cross-impact・backgrounding・handoff の追跡にあり、方法論的診断力は real protocol audit で結果を変えるまで未実証である。
...
したがって旧称の「不在」とは、対象世界からの真理の欠落でも、世界像の必然的非一意性でも、観測の本質的不完全性でもない。**道具的成功に、全ての保証と解釈原理を一括して自己証明する能力がない**という限定的な意味である。v0.3 でも「道具の真理不在性」は失敗履歴を保存する旧作業名としてのみ残す。
...
Bayesian model comparison の M-closed/M-open、statistical misspecification、dimension/leakage diagnosis、causal-model adequacy、Stanford の unconceived alternatives には、それぞれこの問題の一部を扱う field-native vocabulary がある。v0.3 の **candidate-class adequacy** は、それらを置換する新概念ではなく、異なる adequacy problems を横断参照するための索引語に限る。完全対応は `[OPEN: literature audit]` とする。
...
v0.3 では auditability のラベルを増やさず、support provenance を次の七群へ暫定整理する。
...
これは数学的定理ではなく [SYNTHESIS] としての scope-preservation principle である。「道具の真理不在性」は、この ontological non-self-certification を指した旧作業名としてだけ残す。名称自体が真理不存在を連想させる過剰なものだったため、v0.3 の主題名には採用しない。「自己証明されない」と「根拠がない」は同義ではない。
...
この**保証来歴と独立監査可能性**が v0.3 の主問題である。$A_i$ の一部が別の観測や工学的検証によって強く支持されることは否定しないし、その成功を軽視する理由にもならない。
...
「見やすくなった」だけでは方法論的価値を認めない。少なくとも、missing assumption または新しい failure path の発見、experiment design や preregistration の改善、evidence ranking の変更、scope overreach の防止、あるいは別分野での同一 audit procedure の再利用のいずれかを、比較可能な事例で示す必要がある。それまでは、**organizational usefulness plausible; methodological usefulness unproven** と判定する。
...
1. **[OPEN: literature audit] Assurance provenance formalization.** assurance case、metrology、traceability、evidence graph、argumentation framework が既に何を形式化しているか。本ノートのカテゴリーに残る差はあるか。
2. **[OPEN: research] Cross-impact representation.** `one failure → multiple support edges` を、不確実性の二重計上や因果方向の誤認なしにどう表現するか。
3. **[OPEN: research] Handoff loss.** upstream の uncertainty、assumption scope、gauge、selection condition が downstream で実際に失われ、判断を変えた事例はあるか。handoff の存在だけを loss と呼ばない。
4. **[OPEN: research] Backgrounding audit.** 合理的な分業としての backgrounding と、適用範囲を越えてしまう危険な backgrounding を、事後物語ではなく再現可能な基準で区別できるか。
5. **[OPEN: research] Real protocol audit.** loophole-free Bell、quantum tomography、GST の Methods、Supplement、可能なら preregistration を既存 peer review と比較し、missing check を一つでも発見できるか。
6. **[OPEN: research] Erasure benchmark.** standard textbook、review、protocol checklist から audit-map 固有語彙を除いた対照条件と比較し、何が本当に失われるか。
7. **[OPEN: research] Cross-domain reuse.** cosmology、nonequilibrium / irreversible experiments、phylogenetic inference、system identification で同じ handoff / provenance 表現が再現し、実際の判断を改善するか。
...
次に必要なのは新しい概念追加ではなく、**real protocol audit** である。loophole-free Bell experiment、量子 tomography experiment、GST experiment の Methods / Supplement / preregistration を対象に、この map が既存 peer review や標準 checklist で拾われていない missing check、failure path、scope overreach を一つでも発見できるかを対照比較する。何も発見できなければ、その領域での価値は presentation / indexing に留まる。
...
- 保証来歴、backgrounding、handoff、cross-impact を併記する map が、real protocol audit で missing check、failure path、scope overreach の少なくとも一つを発見できる。
...
| **research question generator** | 2 | handoff loss、real protocol audit、erasure benchmark を明示した |
| **audit procedure** | 3 | 候補。real protocol で既存 review を超える診断をまだ示していない |
...
> 観測・予測成功は生成構造を強く支持しうるが、その成功だけでは、それを支えるすべての保証経路と、唯一の存在論へ接続する追加原理を同時には自己証明しない。多くの保証は既存分野で独立に強く支えられており、v0.3 の役割候補は、それらを置換することではなく、保証の出所、scope、backgrounding、handoff、cross-impact を追跡可能にすることである。

**現時点の証拠では、v0.3 は既存科学が照らしていない場所を指すノートではなく、主として、既存の照明がどこから来て、どこへ渡され、どこで合理的に背景化されるかを見えるようにするノートである。** 未知の暗部がないとは言えないが、本調査はそれを確立していない。
