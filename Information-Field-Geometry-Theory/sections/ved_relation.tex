...
\begin{itemize}
  \item \textbf{VED}: Describes the \emph{generation} of physical structure from difference, gradient, flow, and closure. It treats stable structures, particles, fields, and spacetime as quasi-closed or stabilized closure regimes realized through the closure degree $C$.
...
VED addresses the side where closure stabilizes into physical persistence;
IFGT addresses the side where residual difference remains as informational
trace and constraint.
The two are not opposing theories but complementary descriptions of the same
generative structure viewed from different aspects.
This division is visualized in Fig.~\ref{fig:two_aspects}.

\begin{figure}[h]
  \centering
  \includegraphics[width=0.95\textwidth]{figures/fig1_two_aspects_en.pdf}
  \caption{Two aspects of the same generative process. The VED chain (difference $\to$ gradient $\to$ flow $\to$ closure) unfolds across the closure-degree axis $C$. IFGT covers residual quasi-closure where $C$ has not reached unity; VED describes stabilized physical persistence as a quasi-closed regime, not a fully closed state.}
...
\end{equation}
where $f$ is a non-decreasing function satisfying $I \to 0$ as $C \to 1$. The limit $C \to 1$ should be read as an ideal closure boundary rather than an ordinary attainable state. Information is defined in the regime where residual difference remains, i.e. where $C < 1$.
